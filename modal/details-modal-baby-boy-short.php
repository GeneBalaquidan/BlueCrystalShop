<?php
			...
			$num = 0;
			$sql = "SELECT * FROM products p JOIN brand b WHERE p.title = 'Baby Boy Shorts' && p.brand = b.id";
			$featured = $mysqli-> query($sql);
			$product = mysqli_fetch_assoc($featured);
			$num += 1;
?>
<div class="modal fade details-29" id="details-29" tableindex="-1"  role="dialog" aria-labelledby="details-29" aria-hidden="true">
