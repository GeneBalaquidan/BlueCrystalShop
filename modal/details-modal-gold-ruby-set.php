<?php
			...
			$num = 0;
			$sql = "SELECT * FROM products p JOIN brand b WHERE p.title = 'Gold Ruby Set' && p.brand = b.id";
			$featured = $mysqli-> query($sql);
			$product = mysqli_fetch_assoc($featured);
			$num += 1;
?>
<div class="modal fade details-23" id="details-23" tableindex="-1"  role="dialog" aria-labelledby="details-23" aria-hidden="true">
