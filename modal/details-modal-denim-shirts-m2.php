<?php
			...
			$num = 0;
			$sql = "SELECT * FROM products p JOIN brand b WHERE p.title = 'Denim Shirt 2' && p.brand = b.id";
			$featured = $mysqli-> query($sql);
			$product = mysqli_fetch_assoc($featured);
			$num += 1;
?>
<div class="modal fade details-11" id="details-11" tableindex="-1"  role="dialog" aria-labelledby="details-11" aria-hidden="true">
