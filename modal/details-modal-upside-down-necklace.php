<?php
			...
			$num = 0;
			$sql = "SELECT * FROM products p JOIN brand b WHERE p.title = 'Upside Down Necklace' && p.brand = b.id";
			$featured = $mysqli-> query($sql);
			$product = mysqli_fetch_assoc($featured);
			$num += 1;
?>
<div class="modal fade details-14" id="details-14" tableindex="-1"  role="dialog" aria-labelledby="details-14" aria-hidden="true">
