<?php
			...
			$num = 0;
			$sql = "SELECT * FROM products p JOIN brand b WHERE p.title = 'Baby Boy Shorts 2' && p.brand = b.id";
			$featured = $mysqli-> query($sql);
			$product = mysqli_fetch_assoc($featured);
			$num += 1;
?>
<div class="modal fade details-30" id="details-30" tableindex="-1"  role="dialog" aria-labelledby="details-30" aria-hidden="true">
