<?php
			...
			$num = 0;
			$sql = "SELECT * FROM products p JOIN brand b WHERE p.title = 'T-shirt and Short set 5' && p.brand = b.id";
			$featured = $mysqli-> query($sql);
			$product = mysqli_fetch_assoc($featured);
			$num += 1;
?>
<div class="modal fade details-28" id="details-28" tableindex="-1"  role="dialog" aria-labelledby="details-28" aria-hidden="true">
