<?php
			...
			$num = 0;
			$sql = "SELECT * FROM products p JOIN brand b WHERE p.title = 'Brass Lock and Key' && p.brand = b.id";
			$featured = $mysqli-> query($sql);
			$product = mysqli_fetch_assoc($featured);
			$num += 1;
?>
<div class="modal fade details-16" id="details-16" tableindex="-1"  role="dialog" aria-labelledby="details-16" aria-hidden="true">
