<?php
			...
			$num = 0;
			$sql = "SELECT * FROM products p JOIN brand b WHERE p.title = 'T-shirt and Short set' && p.brand = b.id";
			$featured = $mysqli-> query($sql);
			$product = mysqli_fetch_assoc($featured);
			$num += 1;
?>
<div class="modal fade details-24" id="details-24" tableindex="-1"  role="dialog" aria-labelledby="details-24" aria-hidden="true">
