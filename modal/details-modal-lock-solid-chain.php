<?php
			...
			$num = 0;
			$sql = "SELECT * FROM products p JOIN brand b WHERE p.title = 'Lock Solid Chain' && p.brand = b.id";
			$featured = $mysqli-> query($sql);
			$product = mysqli_fetch_assoc($featured);
			$num += 1;
?>
<div class="modal fade details-15" id="details-15" tableindex="-1"  role="dialog" aria-labelledby="details-15" aria-hidden="true">
