<?php
			...
			$num = 0;
			$sql = "SELECT * FROM products p JOIN brand b WHERE p.title = 'Vivienne Necklace' && p.brand = b.id";
			$featured = $mysqli-> query($sql);
			$product = mysqli_fetch_assoc($featured);
			$num += 1;
?>
<div class="modal fade details-18" id="details-18" tableindex="-1"  role="dialog" aria-labelledby="details-18" aria-hidden="true">
