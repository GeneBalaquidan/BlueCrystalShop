<?php
			...
			$num = 0;
			$sql = "SELECT * FROM products p JOIN brand b WHERE p.title = 'Denim Shirt 4' && p.brand = b.id";
			$featured = $mysqli-> query($sql);
			$product = mysqli_fetch_assoc($featured);
			$num += 1;
?>
<div class="modal fade details-13" id="details-13" tableindex="-1"  role="dialog" aria-labelledby="details-13" aria-hidden="true">
