<?php
			...
			$num = 0;
			$sql = "SELECT * FROM products p JOIN brand b WHERE p.title = 'Gold Necklace' && p.brand = b.id";
			$featured = $mysqli-> query($sql);
			$product = mysqli_fetch_assoc($featured);
			$num += 1;
?>
<div class="modal fade details-19" id="details-19" tableindex="-1"  role="dialog" aria-labelledby="details-19" aria-hidden="true">
